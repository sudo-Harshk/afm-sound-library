...
function SoundColumn({ sound, canDelete, onAddReference, onDeleteReference }) {
  ...
  if (!sound) {
    return (
      <div className="flex-1 flex items-center justify-center p-8 text-[13px] text-ink-faint">
  ...
  const submitReference = async (e) => {
    ...
    try {
      ...
      if (err?.message === 'DUPLICATE_URL') return setError('This URL is already added');
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0">
      ...
      <div className="p-5 space-y-5">
        {/* Label details */}
        <div>
          <h3 className="text-[18px] font-semibold text-ink mb-2">{sound.canonicalLabel}</h3>
                    ...
                    <div className="space-y-2">
                      {section.key === 'audio' ? (
                        section.refs.map((ref, i) => (
                          <div key={i} className="w-full px-3 py-2 rounded-lg border border-line">
...
export default function ComparisonPanel({ soundA, soundB, onClose, onAddReference, onDeleteReference, canDelete }) {
  useEffect(() => {
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="flex items-center justify-between px-6 py-4 border-b border-line shrink-0 bg-paper-raised">
        <div className="flex items-center gap-3">
          <span className="material-symbols-outlined text-[20px] text-accent">compare</span>
          <h3 className="text-[18px] font-semibold text-ink">Compare Sounds</h3>
        </div>
        <button
          onClick={onClose}

...
export default function LabelList({ sounds, groupBySubcategory = false, onAddReference }) {
  if (sounds.length === 0) {
  ...
  if (!groupBySubcategory) {
    return (
      <div className="rounded-xl border border-line bg-paper-raised shadow-[var(--shadow-card)] p-2 divide-y divide-line">
        {sounds.map((sound) => (
          <LabelRow key={sound.id} sound={sound} onAddReference={onAddReference} />
        ))}
      </div>
    );
  }
      ...
      {[...groups.entries()].map(([subcategory, items], i) => (
        <div key={subcategory} className={i > 0 ? 'pt-3 mt-2 border-t border-line' : ''}>
          ...
          <div className="divide-y divide-line">
            {items.map((sound) => (
              <LabelRow key={sound.id} sound={sound} showBreadcrumb={false} onAddReference={onAddReference} />
            ))}
          </div>
        </div>
      ))}
